...
export function getSalary(value) {
  const inss_discount = getInssDiscount(value);
  const inss_percentage = value === 0 ? 0 : inss_discount / value;
  const irpf_base = value - inss_discount;
  const irpf_discount = getIrpfDiscount(irpf_base);
  const irpf_percentage = value === 0 ? 0 : irpf_discount / value;
  const net_value = value - inss_discount - irpf_discount;
  const net_percentage = net_value / value;
